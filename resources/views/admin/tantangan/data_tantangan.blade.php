...
@section('content')
  ...
  <div class="page animsition">
    <div class="page-header">
      <h1 class="page-title">Data Tantangan</h1>
      <ol class="breadcrumb">
        <li><a href="{{url('/admin')}}">Beranda</a></li>
        <li class="active">Tantangan</li>
        <li class="active">Data Tantangan</li>
      </ol>
    </div><br>
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          <table class="table table-hover dataTable table-striped width-full" data-plugin="dataTable">
            ...
            <?php $i = 1; ?>
                @foreach ($tantangan as $data)
              <tr>
                <td><?php echo $i; $i+=1; ?></td>
                <td>{{$data->tema_tantangan}}</td>
                <td>{{$data->judul_tantangan}}</td>
                <td>{{$data->deskrpsi_tantangan}}</td>
                <td>{{$data->tenggat_waktu}}</td>
                <td>
                  <div class="widget-header cover overlay overlay-hover">
                    <img class="cover-image overlay-scale" src="{{url('file/contoh_foto/'.$data->contoh_foto)}}" alt="..." style="max-width:50px;max-height:50px;float:left;">
                      <div class="overlay-panel overlay-fade overlay-background overlay-background-fixed text-center vertical-align">
                  </div></div>
                </td>
                <td>
                  <a href="{{route('update_tantangan',$data->id)}}">
                    <button type="button" class="btn btn-icon btn-warning" title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></button>
                  </a>
                  
                  <a href="{{ $data->id }}/destroy">
